    ...
    function drawYMark(ctx,y_max,y_min,line_list_array,flag){
        ...
        for (var i = 0,item; item = line_list_array[i]; i++) {
            ...
            if(this.options.series2 && flag){
                ...
                ctx.textAlign = 'left';
                ctx.fillText(common.format_unit(roundFloat(item.num/1,this.options.data.step2), 2), XYF(this.options.drawWidth + 5), XYF(item.y + 5*dpr));
            }else{
                ctx.textAlign = 'right';
                ctx.fillText(common.format_unit(roundFloat(item.num/1,this.options.data.step), 2), XYF(this.options.padding_left - 5), XYF(item.y + 5*dpr));
            }
        }
         ctx.restore();
    }
