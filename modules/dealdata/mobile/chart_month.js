...
function average(num, data) {
    ...
    for (i = num; i < items.length; i++) {
        ...
        result[i - num].value = parseFloat((countValue / num).toFixed(pricedigit));
    }
...
function dealData(json) {
    ...
    result.timeStrs[0] = transform(arr[0].split(',')[0]);
    result.timeStrs[1] = transform(arr[Math.floor(len/2)].split(',')[0]);
